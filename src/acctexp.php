	...
	if (isset($_POST['username']) && $usage) {
		...
		if (isset($_POST['email'])) {
			if ($mosConfig_uniquemail) {
				...
				$database->setQuery("SELECT id FROM #__users WHERE email = '" . $_POST['email'] . "'");
				if ($database->loadResult()) {
...
function subscriptionDetails ( $option ) {
	...
	if (!$my->id) {
		...
		$database->setQuery("SELECT count(*) FROM #__acctexp_invoices WHERE userid='$my->id'");
		...
		$database->setQuery("SELECT invoice_number, transaction_date, method, amount, params "
		. "\nFROM #__acctexp_invoices"
		. "\nWHERE userid='$my->id' AND active='1' ORDER BY transaction_date DESC LIMIT $min_limit, $rows_limit"
		);
		...
		foreach ($rows as $rowid => $row) {
			if (strcmp($row->transaction_date, "0000-00-00 00:00:00") === 0) {
				...
				$actions = "<a href=\"" .  AECToolbox::deadsureURL("/index.php?option=".$option."&amp;task=repeatPayment&amp;invoice=".$row->invoice_number) . "\">" . _HISTORY_ACTION_REPEAT . "</a>";
							if (is_null($row->fixed)) {
							$actions .= " | "
							. "<a href=\"" . AECToolbox::deadsureURL("/index.php?option=".$option."&amp;task=cancelPayment&amp;invoice=".$row->invoice_number) . "\">" . _HISTORY_ACTION_CANCEL . "</a>";
							}
...
function processNotification ( $option, $processor ) {
	...
	if($id) {
		...
		$short = "Failed Invoice Payment";
		$event = "Processor (" . $processor .") notification for " . $objInvoice->invoice_number . " has failed - invoice number doesn't exist: " . $database->getErrorMsg();
	...
	$short = "Invoice Payment Action";
	$event = "Payment Notification Parser responds:" . "\n";
	...
	if ($response['valid']) {
		...
		if (isset($response['amount_paid'])) {
			if ($response['amount_paid'] != $objInvoice->amount) {
				...
				$event .= " Amount verification failed (" . $response['amount_paid'] . " != " . $objInvoice->amount . "), Payment aborted";
		...
		if (isset($response['amount_currency'])) {
			if ($response['amount_currency'] != $objInvoice->currency) {
				// Amount Fraud, cancel payment and create error log addition
				$event .= " Currency verification failed (" . $response['amount_currency'] . " != " . $objInvoice->currency . "), Payment aborted";
		...
		if (!$break) {
			...
			thanks( $option, $renew, $free );
			$event .= " Payment valid, Action carried out";
			$tags .= ",payment,action";
		}
	} else {
		if ( isset($response['pending']) ) {
			if (strcmp($response['pending_reason'], "signup") === 0) {
				...
				if ($params['trial_free']) {
					...
					$event .= " Payment valid - free trial";
					$tags .= ",payment,action,trial";
				}
			} else {
				$objInvoice->setParams(array("pending_reason" => $response['pending_reason']));
				$event .= " Payment invalid - status is pending, reason: " . $response['pending_reason'];
			...
			$event .= " No Payment - Subscription Cancel";
			$tags .= ",cancel";

			if ($metaUser->objSubscription->hasSubscription) {
				$metaUser->objSubscription->setStatus("Cancelled");
				$event .= ", Userstatus has been updated to 'Cancelled'";
			}
		} elseif (isset($response['eot'])) {
			$metaUser = new metaUser();
			$metaUser->load($objInvoice->userid);
			$event .= " No Payment - Subscription End Of Term";
			$tags .= ",eot";
		} else {
			$event .= " Unknown Error";
			$tags .= ",general_error";
...
function thanks ( $option, $renew, $free ) {
	...
	if($renew) {
		...
		$msg .= "</p><p>";
		$msg .= _SUB_FEPARTICLE_THANKSRENEW;
		if($free) {
			$msg .= _SUB_FEPARTICLE_LOGIN;
		} else {
			$msg .= _SUB_FEPARTICLE_PROCESSPAY;
			$msg .= _SUB_FEPARTICLE_MAIL;
		}
	} else {
		$msg = _SUB_FEPARTICLE_HEAD;
		$msg .= "</p><p>";
		$msg .= _SUB_FEPARTICLE_THANKS;
		if($free) {
