...
class processor_google_checkout extends XMLprocessor
{
	...
	function customtab_details( $request )
	{
		...
		$return .= '<input type="hidden" name="sub" value="google_checkout_details" />' . "\n";
	...
	function customtab_shipping_details( $request )
	{
		...
		$return = '<form action="' . AECToolbox::deadsureURL( 'index.php?option=com_acctexp&amp;task=google_checkout_shipping_details', true ) . '" method="post">' . "\n";
		$return .= $this->getParamsHTML( $var ) . '<br /><br />';
		$return .= $this->getParamsHTML( $var2 ) . '<br /><br />';
		$return .= '<input type="hidden" name="userid" value="' . $request->metaUser->userid . '" />' . "\n";
		$return .= '<input type="hidden" name="task" value="subscriptiondetails" />' . "\n";
		$return .= '<input type="hidden" name="sub" value="google_checkout_shipping_details" />' . "\n";
		$return .= '<input type="submit" class="button" value="' . _BUTTON_APPLY . '" /><br /><br />' . "\n";
		$return .= '</form>' . "\n";
	...
	function validateSubscription( $subscription_id )
	{
		...
		$iFactory = new InvoiceFactory( null, null, null, 'google_checkout' );

		$iFactory->userid = $subscription->userid;
		$iFactory->usage = $invoice->usage;
		$iFactory->processor = 'google_checkout';
