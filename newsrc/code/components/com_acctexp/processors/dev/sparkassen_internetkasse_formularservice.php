...
/**
 * $DOC: Sparkassen-Internetkasse, Anbindung für Händler und Integratoren - Formularservice, Version 2.2.2, 28.10.2010
 * Infos:
 * https://www.bs-card-service.com/de/e-commerce-versandhandel/zahlungsloesungen-im-ueberblick/sparkassen-internetkasse/
 * Technical Infos:
 * https://www.bs-card-service.com/?id=525
 * https://www.bs-card-service.com/fileadmin/user_upload/com-de/Dokumente/02_CONTENT/03_Kundenservice/Downloads/E-Payment/Anbindung_Formularservice_333_Sparkassen_Internetkasse.pdf
 * 
 * 
 * This processor currently works only with
 * - Creditcard paymentes with Mastercard and Visa
 * 
 * This processor has two options:
 * - one time payments 
 *   via the 'Formularservice' (SSL Merchant)
 * - recurring subscriptions/Abonnements)
 *   via the 'Formularservice' and the 'Shopschnittstelle'
 *   (SSL Merchant and Merchant) 
 * 
 * 
 * 
 * Pseudokreditkartennummer (optional setting):
 * -------------------------------------------- 
 * 
 * B+S Card Service (and the Sparkassen-Internet-Kasse Formularservice) seems 
 * to be quite interesting in terms of price and the client does not need to 
 * register, just enters its credit card information once - thats it. - 
 * The service doesn't know anything about the client and AEC doesn't know 
 * anything about the credit card (except the pseudo credit card number, they 
 * do create for you, if needed). For the next payment (in case of recurring 
 * subscriptions) I just need to communicate an amount and the pseudo credit 
 * card number (without knowing any other creditcard details), and they will 
 * charge the client without asking him or AEC for any details. If the payment 
 * is done, AEC sends a note and the invoice to the client. In case of error, 
 * AEC sends an email the Admin or in case the credit card expired present him 
 * a link to the credit card form again. - This is the theory and I am not yet 
 * sure if all this is simple possible to manage. - 
 * 
 * Sparkassen-Internetkasse - Anbindung für Händler und Integratoren - 
 * Formularservice, Version 3.3.3, page 15:
 * 
 * "Pseudokartennummern stellen eine Möglichkeit dar, wiederkehrende Zahlungen
  mit einer Kreditkarte durchführen zu können, ohne die Kreditkartendaten
  in Ihrem System zu speichern. Auf diese Weise müssen Sie nicht am "Payment
  Card Industry Data Security Standard" (PCI DSS) Programm von Visa und MasterCard
  teilnehmen. Ihr Zugang muss für die Nutzung von Pseudokartennummern
  entsprechend konfiguriert sein.
  Wenn Ihr Zugang für die Nutzung von Pseudokartennummern konfiguriert ist,
  können Sie durch Angabe des Parameters payment_options=generate_ppan Sparkassen-
  Internetkasse mitteilen, dass Sie eine Pseudokartennummer für die Kreditkarte
  des Kunden anlegen wollen. In der Shopbenachrichtigung erhalten Sie
  ...
  auch unabhängig von einer Zahlung registrieren. Der Transaktionsablauf entspricht
  dem für Zahlungen ohne 3D-Secure."
 * 
 * 
 * 
 * 
...
class processor_sparkassen_internetkasse_formularservice extends POSTprocessor {
    ...
    function info() {
        ...
        // To 0 for non-recurring, to 1 for recurring and to 2 for providing the administrator with a choice on the matter.
        $info['recurring'] = 1;
        #$info['recurring_buttons']	= 1;  // changes Button type for the user/client

        return $info;
    }

    /**
     * defines standard settings we later may use to contact the payment service
     * 
     * @return Array 
     */
    function settings() {
        ...
        $settings['currency'] = 'EUR'; // IDO 4217 currency code        
    ...
    /**
     * created input fields in Backend  of the AEC processor tab
     * 
     * @return Array 
     */
