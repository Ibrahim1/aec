...
class processor_authorize_cim extends PROFILEprocessor
{
	...
	function checkoutAction( $request, $InvoiceFactory=null )
	{
		...
		$return .= '<button type="submit" class="button aec-btn btn btn-primary' . ( $aecConfig->cfg['checkoutform_jsvalidation'] ? ' validate' : '' ) . '" id="aec-checkout-btn"><i class="icon-shopping-cart iconwhite"></i>' . JText::_('BUTTON_CHECKOUT') . '</button>' . "\n";
		$return .= '</form>' . "\n";
