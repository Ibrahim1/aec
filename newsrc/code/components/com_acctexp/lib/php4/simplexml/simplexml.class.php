...
class simplexml {
	...
	public function array_insert($level, $tags, $value, $type)
	{
		...
		$code = '$this->result' . $temp . "=\"" . addslashes($value) . "\";//(" . $type . ")";
		#echo $code. "\n";
		eval($code);
	}
	 ...
	 */
		...
		elseif (is_array($array) && count($array) == 1 && $array[0]) {
			return $this->array_flatten($array[0]);
		}
		foreach($array as $key => $value) {
			unset($array[$key]);
			$array[$key] = $this->array_flatten($value);
		}
		return $array;
	}
	...
	public function xml_reorganize($array)
	{
		...
		return $this->array_flatten($last);
	}
