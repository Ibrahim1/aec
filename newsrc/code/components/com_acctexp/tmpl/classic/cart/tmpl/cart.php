				...
				<tr>
					<td class="giftdetails">
						<?php if ( !empty( $InvoiceFactory->invoice->params['target_user'] ) ) { ?>
							<p>This purchase will be gifted to: <?php echo $InvoiceFactory->invoice->params['target_username']; ?> (<a href="<?php echo AECToolbox::deadsureURL( 'index.php?option=com_acctexp&task=InvoiceRemoveGiftCart&invoice='.$InvoiceFactory->invoice->invoice_number.'&'. JUtility::getToken() .'=1', $tmpl->cfg['ssl_signup'] ); ?>">undo?</a>)</p>
						<?php } else { ?>
		...
		<tr>
			...
			<div id="confirmation_button">
				...
				if ( !empty( $tmpl->cfg['tos_iframe'] ) && !empty( $tmpl->cfg['tos'] ) ) { ?>
					...
					<p><input name="tos" type="checkbox" /><?php echo sprintf( JText::_('CONFIRM_TOS'), $tmpl->cfg['tos'] ); ?></p>
					<input type="button" onClick="javascript:submitPayment()" class="button" value="<?php echo JText::_('BUTTON_CONFIRM'); ?>" />
					<?php
				} else { ?>
