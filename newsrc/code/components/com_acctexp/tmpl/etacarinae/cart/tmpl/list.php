			...
			<?php foreach ( $InvoiceFactory->cart as $bid => $bitem ) {
				...
				<?php } else { ?>
					<tr>
						<td></td>
						<td></td>
						<td class="center-cell"><button type="submit" class="btn btn-info pull-right"><i class="bsicon-refresh bsiconwhite narrow"></i></button></td>
						<td></td>
						<td class="center-cell"><div id="clear-button"><?php echo $tmpl->lnk( array('task' => 'clearCart'), '<i class="bsicon-trash bsiconwhite narrow"></i>', 'btn btn-danger' ) ?></div></td>
					</tr>
					<tr>
